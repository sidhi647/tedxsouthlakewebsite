...
const event = pastEvents.find((e) => e.year === 2025)!;

export const metadata: Metadata = {
  title: "2025 Conference",
  description: `${event.theme}: ${event.subtheme}. ${event.date} at ${event.venue}.`,
};

export default function Conference2025Page() {
  const speakers = featuredSpeakers.filter((s) => s.year === 2025);

  return (
    <>
      <PageHero
        eyebrow={`Watch · ${event.date}`}
        title={event.theme}
        description={event.subtheme}
      />
      ...
      <Section className="bg-gray-50">
        <SectionHeading eyebrow="Lineup" title="2025 Speakers" />
        {speakers.length === 0 ? (
          <EmptyState message="2025 speaker videos and recap photos are being added to this page." />
        ) : (
